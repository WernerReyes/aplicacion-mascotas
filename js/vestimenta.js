...
( function()  {
// VARIABLES
const educacionalProductos = document.querySelector('#container-vestimenta .container .row')
const contenidoVestimenta = document.querySelector('#container-vestimenta .container');
const inputFiltrar = document.querySelector('.buscar-vestimenta');
...
function eventListeners() {
  contenidoVestimenta.addEventListener( 'click', agregarContenido ); 
  contenidoVestimenta.addEventListener( 'click', mostrarModal );  
...
async function educacionalHTML() {
  const productosVestimenta = await obtenerProductos(2);
  productosHTML(productosVestimenta, 'vestimenta' , educacionalProductos );
}
...
async function mostrarModal(e) {
   ...
   const contenedorModal = document.querySelector('#modal-vestimenta');
   ...
   if(e.target.classList.contains('btn-ver-mas')) {
    ...
    const index = productosVestimenta.findIndex( producto => producto.id === Number(btnVerMas.dataset.id) );
    

    agregarContenidoModal( modalBody, function(contenido){leerDatos(contenido)},
                      productosVestimenta[index], contenido );
...
async function filtrarProductos(tipo) {
  ...
  if( filtrado.length ) {
    productosHTML(filtrado, 'vestimenta', educacionalProductos );
    return;
  } 
