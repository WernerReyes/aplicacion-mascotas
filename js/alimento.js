...
( function()  {
// VARIABLES
const alimentosProductos = document.querySelector('#container-alimentos .container .row')
const contenidoAlimentos = document.querySelector('#container-alimentos .container');
const inputFiltrar = document.querySelector('.buscar-alimento');
...
function eventListeners() {
  contenidoAlimentos.addEventListener( 'click', agregarContenido ); 
  contenidoAlimentos.addEventListener( 'click', mostrarModal );  
...
async function educacionalHTML() {
  const productosAlimento = await obtenerProductos(3);
  productosHTML(productosAlimento, 'alimentos' , alimentosProductos );
}
...
async function mostrarModal(e) {
   ...
   const contenedorModal = document.querySelector('#modal-alimentos');
   ...
   if(e.target.classList.contains('btn-ver-mas')) {
    ...
    const index = productosAlimento.findIndex( producto => producto.id === Number(btnVerMas.dataset.id) );
    

    agregarContenidoModal( modalBody, function(contenido){leerDatos(contenido)},
                      productosAlimento[index], contenido );
...
async function filtrarProductos(tipo) {
  ...
  if( filtrado.length ) {
    productosHTML(filtrado, 'alimento', alimentosProductos );
    return;
  } 
  alertaProductos(alimentosProductos, "No se encontro ningun resultado")
